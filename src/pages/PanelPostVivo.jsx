...
export default function PanelPostVivo() {
  ...
  useEffect(() => {
    ...
    const canal = supabase
      .channel("postvivo-realtime")
      .on(
        "postgres_changes",
        { event: "INSERT", schema: "public", table: "pedidos" },
        () => {
          cargarPedidos(vivoSel);
        },
      )
      .on(
        "postgres_changes",
  ...
  async function cargarPedidos(vivoId) {
    ...
    const { data, error } = await supabase
      ...
      .select(
        ...
        clientas (id, nombre_display, whatsapp, departamento, agencia_preferida, direccion, compras_count),
  ...
  function abrirWA(grupo) {
    ...
    const tienesDatos = !!(
      grupo.clienta?.departamento && grupo.clienta?.agencia_preferida
    );

    const envioBloque = tienesDatos
      ? `🚚 Tu pedido va por *${grupo.clienta.agencia_preferida}* a *${grupo.clienta.departamento}*.`
...
function ModalDatosClienta({ grupo, onGuardar, onClose, guardando }) {
  ...
  const [direccion, setDireccion] = useState(grupo.clienta?.direccion || '')
  ...
  return (
    ...
    >
      <div className="modal">
        ...
        <div className="section-label">Direccion</div>
        <input
          className="input"
          value={direccion}
        ...
        <input
          ...
          placeholder="Ej: OCA, Correo, abitab..."
        />
        <button
