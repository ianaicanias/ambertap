...
function imprimirTicketSamy(pedido, clientaNombre) {
  ...
  ventana.document
    .write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Ticket</title>
...
function ModalNuevaPrenda({ onCrear, onClose }) {
  ...
  function handleCrear() {
    if (!nombre.trim()) {
      setError("Pone el nombre de la prenda");
      return;
    }
    const extras = coloresInput
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean);
      ...
      <div className="modal">
        ...
        >
          {COLORES_DEFAULT.map((c) => (
            <button
              key={c}
              style={cbtn(coloresSel.includes(c))}
              onClick={() =>
                setColoresSel((p) =>
        ...
        >
          {TALLES.map((t) => (
            <button
              key={t}
              style={tbtn(tallesSel.includes(t))}
              onClick={() => {
                setTallesSel((prev) => {
                  if (t === "Unico")
        ...
        <label
          style={{
            ...
            borderRadius: 10,
            border: "1px solid",
            borderColor: enDeposito ? "#C0DD97" : "#c4a0b2",
            background: enDeposito ? "#EAF3DE" : "#f5e6ed",
            cursor: "pointer",
            marginBottom: 14,
            userSelect: "none",
          }}
        >
          <input
            type="checkbox"
            checked={enDeposito}
            onChange={(e) => setEnDeposito(e.target.checked)}
            style={{
              width: 18,
              height: 18,
              cursor: "pointer",
              accentColor: enDeposito ? "#27500A" : "#A0436A",
            }}
          />
          <div>
            <div
              style={{
                fontSize: 14,
                fontWeight: 500,
                color: enDeposito ? "#27500A" : "#A0436A",
              }}
            >
            ...
            <div
              style={{
                fontSize: 11,
                color: enDeposito ? "#3B6D11" : "#7a3351",
                marginTop: 1,
              }}
            >
              {enDeposito
                ? "Mariela prepara el pedido"
                : "Samy entrega y se imprime aca"}
            </div>
          </div>
        </label>
    ...
    >
      <div className="modal">
        ...
        <p style={{ fontSize: 14, color: "#6b4d5a", marginBottom: 14 }}>
          <strong>{nombre}</strong> no esta en la base. Ingresa su WhatsApp para
          registrarla.
        </p>
        ...
        {error && (
          <p style={{ color: "#A32D2D", fontSize: 13, margin: "6px 0" }}>
            {error}
          </p>
        )}
        <button
...
export default function PanelVivo() {
  ...
  async function buscarClientas(q) {
    ...
    const { data } = await supabase
      .from("clientas")
      .select("id, nombre_display, whatsapp, compras_count")
  ...
  async function handleConfirmar() {
    ...
    if (clientaSel) {
      await confirmarPedido(clientaSel);
      return;
    }
    const { data: existe } = await supabase
      .from("clientas")
      .select("id, nombre_display, whatsapp, compras_count")
      .ilike("nombre_display", nombre.trim())
      .maybeSingle();
    if (existe) {
      setClientaSel(existe);
      await confirmarPedido(existe);
    } else {
      setPendingNombre(nombre.trim());
      setModalNuevaClientaNombre(nombre.trim());
    }
  }
  ...
  async function confirmarPedido(clienta) {
    ...
    if (stockConSamy) {
      imprimirTicketSamy(
        { color: colorSel, talle: talleSel, prendaActiva },
        clienta.nombre_display,
      );
    }
  ...
  async function handleNuevaPrenda({
    nombre,
    precio,
    colores,
    talles,
    ...
    const { data: nueva } = await supabase
      ...
      .insert({
        ...
        activa: true,
        orden: prendasAnteriores.length + 1,
        stock_con_samy: stockConSamy,
      })
      .select()
      .single();
    const variantes = [];
    for (const c of colores)
      for (const t of talles)
        variantes.push({ prenda_vivo_id: nueva.id, color: c, talle: t });
    await supabase.from("variantes_prenda").insert(variantes);
    setModalNuevaPrenda(false);
    setColorSel(null);
    setTalleSel(null);
  ...
  async function terminarVivo() {
    if (!vivo) return;
    if (!window.confirm("Terminar el vivo? Samy no podra agregar mas pedidos."))
      return;
    await supabase
      .from("vivos")
    ...
    const horaInicio = new Date(vivo.hora_inicio).toLocaleTimeString("es", {
      hour: "2-digit",
      minute: "2-digit",
    });
    const horaFin = new Date().toLocaleTimeString("es", {
      hour: "2-digit",
      minute: "2-digit",
    });
    ...
    const clientasUnicas = new Set(
      pedidosCompletos.map((p) => p.clientas?.nombre_display).filter(Boolean),
    ).size;
    const totalPotencial = pedidosCompletos.reduce(
      (s, p) => s + (p.prendas_vivo?.precio_unitario || 0),
      0,
    );
    const conSamy = pedidosCompletos.filter(
      (p) => p.prendas_vivo?.stock_con_samy,
    ).length;
    ...
    const topTalles = Object.entries(tallesConteo)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 6);
    const topColores = Object.entries(coloresConteo)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 6);
    const topPrendas = Object.entries(prendasConteo).sort(
      (a, b) => b[1] - a[1],
    );

    const barraHTML = (items, max) =>
      items
        .map(
          ([label, n]) => `
    ...
    const filasTabla = pedidosCompletos
      .map(
        (p, i) => `
  ...
  const coloresActivos = prendaActiva?.variantes_prenda
    ...
    : [];
  const tallesActivos = prendaActiva?.variantes_prenda
    ? [...new Set(prendaActiva.variantes_prenda.map((v) => v.talle))]
    : [];
  const esTalleUnico =
    tallesActivos.length === 1 && tallesActivos[0] === "Unico";
  const incompletos = pedidos.filter((p) => p.incompleto).length;
  const hora = vivo
    ? new Date(vivo.hora_inicio).toLocaleTimeString("es", {
        hour: "2-digit",
        minute: "2-digit",
      })
    : "";
  const fecha = vivo
    ? new Date(vivo.fecha + "T00:00:00").toLocaleDateString("es", {
        day: "2-digit",
        month: "2-digit",
      })
    : "";
  ...
  if (!vivo) {
    return (
      <div style={{ textAlign: "center", paddingTop: 60 }}>
        ...
        <p style={{ fontSize: 14, color: "#9c7d8a", marginBottom: 28 }}>
          Cuando estes listo, iniciá el vivo del dia
        </p>
        <button
          className="btn-primary"
    ...
    <div>
      ...
      >
        <div className="stat-card">
          <div className="num">{pedidos.length}</div>
          <div className="lbl">pedidos</div>
        </div>
        <div className="stat-card">
          <div className="num">{pedidos.length - incompletos}</div>
          <div className="lbl">completos</div>
        </div>
        <div className="stat-card">
          <div
            className="num"
            style={{ color: incompletos > 0 ? "#A32D2D" : "#1a1014" }}
          >
            {incompletos}
          </div>
          <div className="lbl">incompletos</div>
        </div>
      </div>
      ...
      <div
        style={{
          display: "grid",
          gridTemplateColumns:
            prendasAnteriores.length > 0 ? "1fr 160px" : "1fr",
          gap: 12,
          marginBottom: 12,
        }}
      >
        <div>
          {prendaActiva ? (
            <div
              style={{
                background: prendaActiva.stock_con_samy ? "#f5e6ed" : "#EAF3DE",
                border: `1px solid ${prendaActiva.stock_con_samy ? "#c4a0b2" : "#C0DD97"}`,
                borderRadius: 12,
                padding: 14,
              }}
            >
              ...
              >
                <div
                  style={{
                    fontSize: 11,
                    color: prendaActiva.stock_con_samy ? "#A0436A" : "#27500A",
                    fontWeight: 600,
                    letterSpacing: ".06em",
                ...
                <div
                  style={{
                    ...
                    padding: "2px 8px",
                    borderRadius: 99,
                    background: prendaActiva.stock_con_samy
                      ? "#f5e6ed"
                      : "#EAF3DE",
                    color: prendaActiva.stock_con_samy ? "#A0436A" : "#27500A",
                    border: `1px solid ${prendaActiva.stock_con_samy ? "#c4a0b2" : "#C0DD97"}`,
              ...
              <h3
                style={{
                  fontSize: 16,
                  fontWeight: 600,
                  color: prendaActiva.stock_con_samy ? "#7a3351" : "#1a3d1a",
                  marginBottom: 4,
                }}
              >
                {prendaActiva.nombre}
              ...
              >
                {prendaActiva.precio_unitario
                  ? "$" + prendaActiva.precio_unitario.toLocaleString("es")
                  : "sin precio"}
              </div>
                ...
                <>
                  ...
                  >
                    {tallesActivos.map((t) => (
                      <button
                        key={t}
                        style={tbtn(talleSel === t)}
                        onClick={() => setTalleSel(talleSel === t ? null : t)}
                      >
                        {t}
                      </button>
                    ))}
              ...
              <div style={{ position: "relative" }} ref={sugerRef}>
                <input
                  className={`input ${errorNombre ? "error" : ""}`}
                  placeholder="Nombre de la clienta..."
                  value={nombre}
                  onChange={(e) => buscarClientas(e.target.value)}
                  onKeyDown={(e) => e.key === "Enter" && handleConfirmar()}
                  autoComplete="off"
                  style={{ marginBottom: clientaSel ? 4 : 8 }}
                />
                {clientaSel && (
                  <div
                    style={{ fontSize: 12, color: "#27500A", marginBottom: 8 }}
                  >
                    Clienta conocida · {clientaSel.compras_count} compras
                    anteriores
                  </div>
                )}
                  ...
                  >
                    {sugerencias.map((c) => (
                      <div
                        ...
                        onMouseEnter={(e) =>
                          (e.currentTarget.style.background = "#f7f3f5")
                        }
                        onMouseLeave={(e) =>
              ...
              {!colorSel && !talleSel && nombre && (
                <p style={{ fontSize: 12, color: "#854F0B", marginBottom: 8 }}>
                  Sin color ni talle — se guarda como incompleto
                </p>
              )}
              {prendaActiva.stock_con_samy && (
                <p
                ...
                >
                  El pedido queda preparado y se imprime el ticket aca
                </p>
              )}
              <button
                className="btn-primary"
            ...
            >
              <p style={{ color: "#A0436A", marginBottom: 12 }}>
                No hay prenda activa
              </p>
              <button
                className="btn-primary"
                onClick={() => setModalNuevaPrenda(true)}
              >
              ...
              >
                <div style={{ flex: 1 }}>
                  ...
                  <div
                    style={{
                      fontSize: 12,
                      color: p.incompleto ? "#A32D2D" : "#9c7d8a",
                    }}
                  >
                ...
                <span style={{ fontSize: 11, color: "#9c7d8a", marginLeft: 8 }}>
                  {new Date(p.hora).toLocaleTimeString("es", {
                    hour: "2-digit",
                    minute: "2-digit",
                  })}
                </span>
                  ...
                  <div style={{ display: "flex", gap: 4 }}>
                    ...
                    <span
                      style={{
                        ...
                        fontWeight: 500,
                        borderColor:
                          p.estado === "preparado" ? "#C0DD97" : "#e8dde3",
                        background:
                          p.estado === "preparado" ? "#EAF3DE" : "#f7f3f5",
                        color: p.estado === "preparado" ? "#27500A" : "#9c7d8a",
                      }}
                    >
                      {p.estado === "preparado" ? "preparado" : "pendiente"}
                    </span>
                  </div>
                  {p.estado === "pendiente" && (
                    <button
      ...
      {modalNuevaClientaNombre && (
        <ModalClientaNueva
          nombre={modalNuevaClientaNombre}
          onGuardar={handleClientaNuevaGuardar}
          onClose={() => setModalNuevaClientaNombre(null)}
        />
      )}
    </div>
  );
