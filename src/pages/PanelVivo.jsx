            ...
            <button
              ...
              onClick={() => {
                setTallesSel((prev) => {
                  // si selecciona Unico
                  if (t === "Unico") {
                    if (prev.includes("Unico")) return [];
                    return ["Unico"];
                  }

                  // si ya estaba unico, lo sacamos
                  let nuevos = prev.filter((x) => x !== "Unico");

                  if (nuevos.includes(t)) {
                    return nuevos.filter((x) => x !== t);
                  } else {
                    return [...nuevos, t];
                  }
                });
...
export default function PanelVivo() {
  ...
  const [vivoTerminado, setVivoTerminado] = useState(false);
  ...
  async function handleNuevaPrenda({ nombre, precio, colores, talles }) {
    if (!vivo) return;
    await supabase
      .from("prendas_vivo")
      .update({ activa: false })
  ...
  async function terminarVivo() {
    ...
    const lineas = [...pedidos]
      ...
      .map(
        (p) =>
          `${new Date(p.hora).toLocaleTimeString("es", { hour: "2-digit", minute: "2-digit" })} | ${p.clientas?.nombre_display || "—"} | ${p.prendas_vivo?.nombre || "—"} | ${p.color || "—"} ${p.talle || "—"}${p.incompleto ? " [INCOMPLETO]" : ""}`,
      )
      .join("\n");
  ...
  return (
    <div>
      ...
      >
        ...
        <div className="stat-card">
          <div class="num">{pedidos.length - incompletos}</div>
      ...
      {prendaActiva ? (
        ...
        >
          ...
          <div style={{ fontSize: 13, color: "#A0436A", marginBottom: 12 }}>
      ...
      {prendasAnteriores.length > 0 && (
        <>
          ...
          {prendasAnteriores.map((p) => (
            ...
            >
              <span style={{ fontSize: 13, color: "#6b4d5a" }}>
                {p.nombre}
                {p.precio_unitario
          ...
          <div className="card" style={{ padding: "4px 14px" }}>
            {pedidos.map((p) => (
              ...
              >
                ...
                {p.incompleto ? (
                  <span className="badge-incompleto" style={{ marginLeft: 6 }}>
                    inc
                  </span>
                ) : (
