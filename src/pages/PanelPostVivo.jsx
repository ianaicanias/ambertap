...
export default function PanelPostVivo() {
  ...
  useEffect(() => {
    if (vivoSel) cargarPedidos(vivoSel);
  }, [vivoSel]);
  ...
  async function cargarVivos() {
    ...
    if (error) {
      setError("Error cargando vivos: " + error.message);
      setLoading(false);
      return;
    }
    setVivos(data || []);
...
Un abrazo de parte del equipo AmberTap 💙`;
    const tel = grupo.clienta?.whatsapp?.replace(/\D/g, "") || "";
    const url = tel
  ...
  const datoCompleto = (g) =>
    !!(g.clienta?.whatsapp && g.clienta?.departamento);
  ...
  const vivosPorFecha = vivos.reduce((acc, v) => {
    ...
    <div>
      ...
      {vivos.length > 0 && (
        <div style={{ marginBottom: 14 }}>
          ...
          >
            {Object.keys(vivosPorFecha).map((fecha) => {
              const vivosDelDia = vivosPorFecha[fecha];
              const seleccionadoEnEsteDia = vivosDelDia.some(
                (v) => v.id === vivoSel,
              );
              return (
                <button
                  key={fecha}
                  onClick={() => {
                    const mas_reciente = vivosDelDia[0];
                    setVivoSel(mas_reciente.id);
                    setFechaSel(fecha);
                  }}
                  style={{
                    padding: "5px 12px",
                    ...
                    fontSize: 12,
                    cursor: "pointer",
                    borderColor: seleccionadoEnEsteDia ? "#A0436A" : "#e8dde3",
                    background: seleccionadoEnEsteDia ? "#f5e6ed" : "#fff",
                    color: seleccionadoEnEsteDia ? "#A0436A" : "#9c7d8a",
                    fontWeight: seleccionadoEnEsteDia ? 500 : 400,
                  }}
                >
                  {formatFecha(fecha)}
                </button>
          ...
          {fechaSel &&
            vivosPorFecha[fechaSel] &&
            vivosPorFecha[fechaSel].length > 1 && (
              <div
                style={{
                  display: "flex",
                  gap: 6,
                  flexWrap: "wrap",
                  paddingLeft: 8,
                  borderLeft: "2px solid #f5e6ed",
                }}
              >
                {[...vivosPorFecha[fechaSel]].sort((a, b) => new Date(a.hora_inicio) - new Date(b.hora_inicio)).map(v => {
                  const hora = new Date(v.hora_inicio).toLocaleTimeString(
                    "es",
      ...
      >
        <button style={fbStyle("todas")} onClick={() => setFiltro("todas")}>
          Todas ({grupos.length})
        </button>
        <button
          style={fbStyle("sinpagar")}
          onClick={() => setFiltro("sinpagar")}
        >
          Sin pagar
        </button>
        <button
          style={fbStyle("incompleto")}
          onClick={() => setFiltro("incompleto")}
        >
          Datos incompletos
        </button>
        <button
          style={fbStyle("completo")}
        ...
        {gruposFiltrados.map((g, i) => {
          ...
          return (
            <div
              ...
              style={{
                background: "#fff",
                border: `1px solid ${completo ? "#e8dde3" : "#FAC775"}`,
                borderRadius: 12,
                padding: "14px 16px",
              }}
            >
              ...
              >
                <div
                  style={{
                    ...
                    fontWeight: 600,
                    flexShrink: 0,
                    ...avatarColor(g.clienta?.nombre_display),
                  }}
                >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 14, fontWeight: 600 }}>
                    {g.clienta?.nombre_display || "Sin nombre"}
                  </div>
                  <div style={{ fontSize: 12, color: "#9c7d8a" }}>
                    {g.pedidos.length}{" "}
                    {g.pedidos.length === 1 ? "prenda" : "prendas"}
                    {total > 0 ? ` · $${total.toLocaleString("es")}` : ""}
                    {g.clienta?.whatsapp
                      ? ` · +598 ${g.clienta.whatsapp}`
                      : " · sin WA"}
                  </div>
                  {g.clienta?.departamento && (
                    <div style={{ fontSize: 12, color: "#9c7d8a" }}>
                      {g.clienta.departamento}
                      {g.clienta.agencia_preferida
                        ? ` · ${g.clienta.agencia_preferida}`
                        : ""}
                    </div>
                  )}
              ...
              >
                {g.pedidos.map((p) => (
                  <div
                    ...
                    style={{
                      fontSize: 13,
                      color: p.incompleto ? "#A32D2D" : "#6b4d5a",
                      padding: "3px 0",
                      display: "flex",
                      gap: 8,
                    }}
                  >
                    <span>·</span>
                    <span>
                      {p.prendas_vivo?.nombre || "—"} — {p.color || "—"}{" "}
                      {p.talle || "—"}
                      {p.prendas_vivo?.precio_unitario
                        ? ` ($${p.prendas_vivo.precio_unitario.toLocaleString("es")})`
                        : ""}
      ...
      {modalWA && (
        <div
          className="overlay"
          onClick={(e) => e.target === e.currentTarget && setModalWA(null)}
        >
          <div className="modal">
            <h3>
              WA — {modalWA.nombre}{" "}
              <button className="modal-close" onClick={() => setModalWA(null)}>
                &#x2715;
              </button>
            </h3>
            <pre
...
function ModalDatosClienta({ grupo, onGuardar, onClose, guardando }) {
  ...
  const [departamento, setDepartamento] = useState(
    grupo.clienta?.departamento || "",
  );
  const [agencia, setAgencia] = useState(
    grupo.clienta?.agencia_preferida || "",
  );
  ...
  return (
    ...
    >
      <div className="modal">
        <h3>
          Datos de {grupo.clienta?.nombre_display}{" "}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{ fontSize: 14, color: "#9c7d8a", flexShrink: 0 }}>
            +598
          </span>
          <input
            className="input"
        ...
        <input
          ...
          placeholder="Ej: DAC, correo, Copay..."
        />
        <button
          className="btn-primary"
          disabled={guardando}
          onClick={() =>
            onGuardar(grupo.clienta?.id, {
